...
func TestActorEgressMITMTrust(t *testing.T) {
	if os.Getenv("E2E_EGRESS_MITM") == "" {
		t.Skip("needs the sdsmint (MITM) egress gateway: deploy with hack/install-ate-kind.sh --deploy-atenet --deploy-ate-apiserver --experimental-use-sdsmint, then set E2E_EGRESS_MITM=1")
	}
	...
	waitForGolden(t, ctx, clients, probeNamespace)
...
func TestActorEgressTrustAutoInjection(t *testing.T) {
	if os.Getenv("E2E_EGRESS_MITM") == "" {
		t.Skip("needs the sdsmint (MITM) egress gateway and injection: deploy with hack/install-ate-kind.sh --deploy-atenet --deploy-ate-apiserver --experimental-use-sdsmint, then set E2E_EGRESS_MITM=1")
	}
	env, err := e2e.CheckEnv("BUCKET_NAME", "KO_DOCKER_REPO")
	if err != nil {
		t.Fatalf("CheckEnv failed: %v", err)
	...
	ns := e2e.DeployProbe(t, env["BUCKET_NAME"], "inject")
	...
	if read.Error != "" {
		t.Fatalf("reading %s in the actor: %s — is ateapi running with --inject-egress-trust-bundle? (redeploy with hack/install-ate-kind.sh --deploy-ate-apiserver --experimental-use-sdsmint)", injectedPath, read.Error)
	}
...
func waitForGolden(t *testing.T, ctx context.Context, clients *e2e.Clients, ns string) {
	...
	for time.Now().Before(deadline) {
		at, err := clients.SubstrateK8s.ApiV1alpha1().ActorTemplates(ns).Get(ctx, probeTemplate, metav1.GetOptions{})
		if err == nil {
...
func createAndResumeActor(t *testing.T, ctx context.Context, clients *e2e.Clients, ns, id string) {
	t.Helper()
	_, _ = clients.SubstrateAPI.CreateAtespace(ctx, &ateapipb.CreateAtespaceRequest{Atespace: &ateapipb.Atespace{Metadata: &ateapipb.ResourceMetadata{Name: ns}}})
